...
def tagBuilder(options, p, E, eta, phi, ranges, delta):
    outTag = ""
    printOut = "{}{}".format(col.bold, col.yellow)
    if options.closeBy:
        outTag = "CloseBy"
    ...
    if options.nParticles == 1:
        outTag = "{}Single{}".format(outTag, particleTag)
    elif options.nParticles == 2:
        outTag = "{}Double{}".format(outTag, particleTag)
    else:
        outTag = "{}Multi{}".format(outTag, particleTag)
    ...
    if E == "notSet":
        outTag = "{}_E{}to{}".format(outTag, makeTag(ranges[0]), makeTag(ranges[1]))
        printOut = "{}E in ({},{}) GeV, ".format(
            printOut,
            makeTag(ranges[0]),
            makeTag(ranges[1]),
        )
    else:
        outTag = "{}_E{}".format(outTag, makeTag(E))
        printOut = "{}E={} GeV, ".format(printOut, makeTag(E))
    if eta == "notSet":
        outTag = "{}Eta{}to{}".format(outTag, makeTag(ranges[2]), makeTag(ranges[3]))
        printOut = "{}eta in ({},{}), ".format(
            printOut,
            makeTag(ranges[2]),
            makeTag(ranges[3]),
        )
    else:
        outTag = "{}Eta{}".format(outTag, makeTag(eta))
        printOut = "{}eta={}, ".format(printOut, makeTag(eta))
    if phi == "notSet":
        if options.minPhi is not None or options.maxPhi is not None:
            outTag = "{}Phi{}to{}".format(outTag, makeTag(ranges[4]), makeTag(ranges[5]))
        printOut = "{}and phi in ({},{}){}".format(
            printOut,
            makeTag(ranges[4]),
            makeTag(ranges[5]),
            col.endc,
        ...
        outTag = "{}Phi{}".format(outTag, makeTag(phi))
        printOut = "{}and phi={}{}".format(printOut, makeTag(phi), col.endc)
    if delta != None:
        outTag = "{}Delta{}".format(outTag, makeTag(delta))
...
def writeCRABConfig(
    ...
    file1 = open("crabConfig_{}_{}.py".format(outTag, options.step), "w")
    requestName = "{}_{}_upgrade2026_{}".format(outTag, CMSSW, options.geometry)
    if options.campaign is not None:
        requestName = "{}_{}".format(requestName, options.campaign)
    outputDatasetTag = requestName
    if options.tag is not None:
        requestName = "{}_{}".format(requestName, options.tag)
    requestName = "'{}_{}'\n".format(requestName, options.step)
    outputDatasetTag = "'{}_{}'\n\n".format(outputDatasetTag, options.step)
    ...
    file1.write("config.Data.outLFNDirBase = '{}{}/'\n".format(options.dest, USER))
...
def fetchData(options, energy, particle, eta, phi, ranges, delta, CMSSW, user):
    ...
    # output dataset
    output = "{}_{}_upgrade2026_{}".format(primary, CMSSW, options.geometry)
    if options.campaign is not None:
        output = "{}_{}".format(output, options.campaign)
    ...
    # dasgoclient command
    query = "dataset dataset=/{}/{}-{}-*/USER instance=prod/phys03".format(
        primary,
        user,
        output,
    )
